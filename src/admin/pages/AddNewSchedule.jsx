...
import { getAllMovieNonePhotoAPI, getMovieByIdAPI } from '~/apis/movieApi'
import { getListBranchNonePhotoAPI } from '~/apis/branchApi'
...
export default function AddNewSchedule() {
	...
	useEffect(() => {
		getAllMovieNonePhotoAPI().then(res => setListMovie(res))
		getListBranchNonePhotoAPI().then(res => setListBranch(res))
	}, [])
		...
		<Box
			...
			<Box>
				<form onSubmit={handleSubmit}>
					<Box sx={{ display:'flex', gap:1, fontSize:18, mb:'8px', alignItems:'center', width:'100%', justifyContent:'center' }}>
						<Box sx={{ display:'flex', alignItems:'center', mr:'10px' }}>
							<Box sx={{ width:'60px', marginLeft:'20px' }}>Date:</Box>
							<Box >
               ...
               <Box sx={{ display:'flex', ml:'10px', alignItems:'center', justifyContent:'center' }}>
               	<Box>Duration:</Box>
               	<Box sx={{marginLeft:'10px'}}> {Math.floor(movie.duration / 60) + ' Hour ' + movie.duration % 60 + ' Minute '} </Box>
               	<Box sx={{marginLeft:'100px'}}>Release date:</Box>
               	<Box sx={{marginLeft:'10px'}}> {movie.releaseDate} </Box>
               </Box>
					}
